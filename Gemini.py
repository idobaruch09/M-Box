...
def search_links(txt):
    try:
        client = genai.Client(api_key=key)
        request = "\"" + txt + "\"\n\nSearch for real links, answer in the format of:\nlink1,link2,link3...\n dont add links that dont appear, even if there is just one. make sure all chars are actually related. dont add links that arent here even if there is none. if there are no links your response will be ' '. think, make sure that you are right, and just then answer."
